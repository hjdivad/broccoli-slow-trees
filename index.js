...
function printSlowNodes(tree, factor) {
  try {
    ...
    for (var i = 0; i < summary.nodes.length; i++) {
      var nw = summary.nodes[i];

      if (nw.selfTime > msThreshold) {
        logLines.push(pad(ellipsize(nw.label, MAX_NAME_CELL_LENGTH), MAX_NAME_CELL_LENGTH) + ' | ' + pad(Math.floor(nw.selfTime) + 'ms', MAX_VALUE_CELL_LENGTH));
      }
    }

    for (var i=0; i<summary.groupedNodes.length; ++i) {
      if (summary.groupedNodes[i].count > 1) {
        includeGroup = true;
        break;
      }
    }
